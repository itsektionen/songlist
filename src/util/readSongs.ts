...
import { JSON_SONGS_PATH, XML_NO_IDS_SONGS_PATH, XML_SONGS_PATH } from '../definitions/paths';
...
export async function readSongsXmlNoIds() {
	return await readFile(XML_NO_IDS_SONGS_PATH).then((file) => file.toString());
}
...
export default async function readSongs() {
	return {
		...
		xmlNoIds: await readSongsXmlNoIds(),
	};
}
